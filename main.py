...
def home_page():
    ...
    c5, c6 = st.columns(2)
    with c5:
        if st.button("🧾 아이센서 유지보수관리", use_container_width=True, key="home_btn_maintenance"):
            st.session_state.menu = "아이센서 유지보수관리"
            st.rerun()
            return
    with c6:
        st.empty()
    ...
    • 이제 통계 대시보드에서 실사 / 시공 / 연차 / 유지보수 현황을 한 번에 볼 수 있습니다.
    </div>
    """, unsafe_allow_html=True)
...
if not st.session_state.logged_in:
    ...
    if st.session_state.menu == "홈":
        ...
        inspection_page()
    elif st.session_state.menu == "아이센서 유지보수관리":
        maintenance_page()
